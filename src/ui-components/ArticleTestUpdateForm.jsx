...
import { getArticleTest } from "../graphql/queries";
import { updateArticleTest } from "../graphql/mutations";
function ArrayField({
  items = [],
  onChange,
  label,
...
export default function ArticleTestUpdateForm(props) {
  const {
    id: idProp,
    articleTest: articleTestModelProp,
    onSuccess,
    onError,
    onSubmit,
    onValidate,
  ...
  const resetStateValues = () => {
    const cleanValues = articleTestRecord
      ? { ...initialValues, ...articleTestRecord }
      : initialValues;
    setImage(cleanValues.image);
    setSeasons(cleanValues.seasons ?? []);
    setCurrentSeasonsValue("");
    setUsage(cleanValues.usage);
    setCreatedAt(cleanValues.createdAt);
    setOwner(cleanValues.owner);
    setErrors({});
  };
  const [articleTestRecord, setArticleTestRecord] =
    React.useState(articleTestModelProp);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? (
            await API.graphql({
              query: getArticleTest.replaceAll("__typename", ""),
              variables: { id: idProp },
            })
          )?.data?.getArticleTest
        : articleTestModelProp;
      setArticleTestRecord(record);
    };
    queryData();
  }, [idProp, articleTestModelProp]);
  React.useEffect(resetStateValues, [articleTestRecord]);
    ...
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await API.graphql({
            query: updateArticleTest.replaceAll("__typename", ""),
            variables: {
              input: {
                id: articleTestRecord.id,
                ...modelFields,
              },
            },
          });
      ...
      {...getOverrideProps(overrides, "ArticleTestUpdateForm")}
      {...rest}
    >
      <TextField
        label="Image"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || articleTestModelProp)}
